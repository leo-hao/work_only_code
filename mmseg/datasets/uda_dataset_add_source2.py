...
class UDADataset(object):
    ...
    def __init__(self, source, source2, target, cfg):
        self.source = source
        # ------------------------
        #！！！！！
        ...
        # ------------------------
        #!!!!!新增一个source2
        assert target.ignore_index == source2.ignore_index
        assert target.CLASSES == source2.CLASSES
        assert target.PALETTE == source2.PALETTE
    ...
    def get_rare_class_sample(self):
        ...
        # ------------------------
        c12 = np.random.choice(self.rcs_classes, p=self.rcs_classprob)
        f12 = np.random.choice(self.samples_with_class[c12])
        i12 = self.file_to_idx[f12]
        s12 = self.source2[i12]
        # ------------------------
        ...
        if self.rcs_min_crop_ratio > 0:
            for j in range(10):
                n_class = torch.sum(s12['gt_semantic_seg'].data == c12)
                # mmcv.print_log(f'{j}: {n_class}', 'mmseg')
                if n_class > self.rcs_min_pixels * self.rcs_min_crop_ratio:
                    break
                s12 = self.source2[i12] 
        ...
        return {
            # **s1, 'img_metas2’':s12['img_metas'], 'img2': s12['img'], 'gt_semantic_seg2': s12['gt_semantic_seg'], 'target_img_metas': s2['img_metas'],
            # 'target_img': s2['img']
        }
    ...
    def __getitem__(self, idx):
        if self.rcs_enabled:
            ...
            s12 = self.source2[idx // len(self.target)]
