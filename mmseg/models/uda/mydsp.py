...
class DSP(UDADecorator):
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg, img2, img_metas2, gt_semantic_seg2, target_img,
        ...
        # 从source2中取mask
        mix_masks = get_class_masks(gt_semantic_seg2)
        #print('-----------------')
        ...
        for i in range(batch_size):
            ...
            if self.Ll_soft:
                ...
                mixed_img_s[i], mixed_lbl_s[i] = strong_transform(
                    strong_parameters,
                    data=torch.stack((img2[i], img[i])),
                    target=torch.stack((gt_semantic_seg2[i][0], gt_semantic_seg[i][0])))
        ...
        if self.mmd_loss:
            ...
            mmd_loss.backward()
            log_vars.update(add_prefix(mmd_log, 'leo'))
